...
# この段階ではまだ存在しないモジュール
from src.parsers.seekingalpha import parse_seekingalpha_portfolio
...
class TestSeekingAlphaParser:
    ...
    def parsed_data(self):
        ...
        return parse_seekingalpha_portfolio(SAMPLE_FILE)
    ...
    def test_all_sheets_parsed(self, parsed_data):
        ...
        # Summary
        assert aapl_data.platform_data.price is not None
        # Ratings
        assert aapl_data.platform_data.valuation_grade is not None
        # Holdings (サンプルでは空のはず)
        assert aapl_data.platform_data.shares is None
        # Dividends
        assert aapl_data.platform_data.dividend_safety is not None

    def test_empty_values_handled(self, parsed_data):
        """'-'や空値がNoneに変換されることをテスト"""
        # Holdingsシートのデータはほぼすべて'-'なので、Noneになっているはず
        aapl_data = next((s for s in parsed_data if s.symbol == "AAPL"), None)
        assert aapl_data.platform_data.cost is None
        assert aapl_data.platform_data.value is None

    def test_rating_values_retained(self, parsed_data):
        """レーティング（A+, Fなど）が正しく文字列として保持されることをテスト"""
        aapl_data = next((s for s in parsed_data if s.symbol == "AAPL"), None)
        assert aapl_data.platform_data.valuation_grade == "F"
        assert aapl_data.platform_data.profitability_grade == "A+"

    def test_data_model_structure(self, parsed_data):
        """データモデルの構造が正しいことをテスト"""
        sample_stock = parsed_data[0]
        assert isinstance(sample_stock, StockData)
        assert isinstance(sample_stock.platform_data, SeekingAlphaData)
        assert sample_stock.symbol is not None
        # Seeking Alphaのデータには取引所情報がないためNone
        assert sample_stock.exchange is None
        assert sample_stock.full_symbol == sample_stock.symbol
