...
# この段階ではまだ存在しないモジュール
from src.parsers.tradingview import parse_tradingview_watchlist
...
class TestTradingViewParser:
    ...
    def parsed_data(self):
        ...
        return parse_tradingview_watchlist(SAMPLE_FILE)
    ...
    def test_section_information_retained(self, parsed_data):
        ...
        section_1_stocks = [s for s in parsed_data if s.platform_data.section == "###SECTION 1"]
        section_2_stocks = [s for s in parsed_data if s.platform_data.section == "###SECTION 2"]
        ...
        section_none_stocks = [s for s in parsed_data if s.platform_data.section is None]
