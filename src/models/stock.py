...
class TradingViewData(PlatformData):
    ...
    section: Optional[str] = Field(None, description="TradingViewのセクション名")
...
class SeekingAlphaData(PlatformData):
    ...
    # Summaryシートから
    price: Optional[float] = Field(None, description="現在価格")
    ...
    avg_volume: Optional[float] = Field(None, description="平均出来高")
    day_low: Optional[float] = Field(None, description="日中安値")
    day_high: Optional[float] = Field(None, description="日中高値")
    week52_low: Optional[float] = Field(None, description="52週安値")
    week52_high: Optional[float] = Field(None, description="52週高値")
    quant_rating: Optional[float] = Field(None, description="Quantレーティング")
    sa_analyst_rating: Optional[float] = Field(None, description="SAアナリストレーティング")
    wall_street_rating: Optional[float] = Field(None, description="ウォールストリートレーティング")

    # Ratingsシートから
    valuation_grade: Optional[str] = Field(None, description="バリュエーショングレード")
    growth_grade: Optional[str] = Field(None, description="グロースグレード")
    profitability_grade: Optional[str] = Field(None, description="収益性グレード")
    momentum_grade: Optional[str] = Field(None, description="モメンタムグレード")
    eps_revision_grade: Optional[str] = Field(None, description="EPSリビジョングレード")

    # Holdingsシートから
    shares: Optional[float] = Field(None, description="保有株数")
    cost: Optional[float] = Field(None, description="取得原価")
    todays_gain: Optional[float] = Field(None, description="本日の損益")
    todays_gain_percent: Optional[float] = Field(None, description="本日の損益%")
    total_change: Optional[float] = Field(None, description="総損益")
    total_change_percent: Optional[float] = Field(None, description="総損益%")
    value: Optional[float] = Field(None, description="現在価値")

    # Dividendsシートから
    dividend_safety: Optional[str] = Field(None, description="配当安全性")
    dividend_growth: Optional[str] = Field(None, description="配当成長性")
    dividend_yield_grade: Optional[str] = Field(None, description="配当利回りグレード")
    dividend_consistency: Optional[str] = Field(None, description="配当一貫性")
    ex_dividend_date: Optional[str] = Field(None, description="配当落ち日")
    payout_date: Optional[str] = Field(None, description="支払日")
    frequency: Optional[str] = Field(None, description="配当頻度")
    yield_ttm: Optional[float] = Field(None, description="配当利回りTTM")
    yield_forward: Optional[float] = Field(None, description="配当利回りFWD")
    dividend_rate_ttm: Optional[float] = Field(None, description="配当率TTM")
    dividend_rate_forward: Optional[float] = Field(None, description="配当率FWD")
    payout_ratio: Optional[float] = Field(None, description="配当性向")
    dividend_growth_3y: Optional[float] = Field(None, description="配当成長率3年")
    dividend_growth_5y: Optional[float] = Field(None, description="配当成長率5年")
    beta_24m: Optional[float] = Field(None, description="24ヶ月ベータ")
...
class StockData(BaseModel):
    ...
    # TradingView固有のデータ
    tradingview_section: Optional[str] = Field(None, description="TradingViewのセクション名")

    # 共通データ
    source_platform: Optional[str] = Field(None, description="データソースプラットフォーム")
    date_added: Optional[datetime] = Field(None, description="追加日")
    date_updated: Optional[datetime] = Field(None, description="更新日")
    notes: Optional[str] = Field(None, description="メモ")
    status: Optional[str] = Field("active", description="ステータas (active/inactive)")
    ...
    def validate_symbol(cls, v: str) -> str:
        ...
        return v.strip().upper()  # シンボルは常に大文字に正規化
    ...
    def validate_exchange(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is None:
            return None
        return v.strip().upper() if v else None  # 取引所名は常に大文字に正規化
